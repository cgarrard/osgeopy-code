...
#####################  6.4  Taking advantage of filters  #######################

# Use a filter and CopyLayer to easily copy all African countries to a new
# dataset.

shp_fn = os.path.join(data_dir, 'global', 'ne_10m_admin_0_countries.shp')
json_fn = os.path.join(data_dir, 'output', 'africa-filter.geojson')

# Open the global countries shapefile and set a filter for Africa.
shp_ds = ogr.Open(shp_fn)
shp_lyr = shp_ds.GetLayer(0)
shp_lyr.SetAttributeFilter("CONTINENT = 'Africa'")

# Create a new GeoJSON data source.
json_ds = ogr.GetDriverByName('GeoJSON').CreateDataSource(json_fn)

# Optional layer creation option.
lyr_options = ['COORDINATE_PRECISION=6']

# Copy the filtered features in the shapefile into a new layer called
# africe-filter in the GoeJSON data source.
json_lyr = json_ds.CopyLayer(shp_lyr, 'africa-filter', lyr_options)


# Only copy a subset of the attributes by using a SQL query. Get the layer to
# copy using ExecuteSQL, but other than that, it's the same.
shp_ds = ogr.Open(os.path.join(data_dir, 'global'))
sql = """SELECT NAME, POP_EST FROM ne_10m_admin_0_countries
         WHERE CONTINENT = 'Africa'"""
shp_lyr = shp_ds.ExecuteSQL(sql)

json_fn = os.path.join(data_dir, 'output', 'africa-filter2.geojson')
json_ds = ogr.GetDriverByName('GeoJSON').CreateDataSource(json_fn)
lyr_options = ['COORDINATE_PRECISION=6']
json_lyr = json_ds.CopyLayer(shp_lyr, 'africa-filter2', lyr_options)
